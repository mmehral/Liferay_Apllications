...
public class KnowledgeBitesAction extends MVCPortlet{
	private static final Log logger = LogFactoryUtil.getLog(KnowledgeBitesAction.class);
	...
	public void save(ActionRequest actionRequest, ActionResponse actionResponse)
		...
		if(buinessDesc.equalsIgnoreCase("") || buinessDesc.length()<1)
		{
			System.out.println("ck editor data null>>>>>>");
			SessionErrors.add(actionRequest, "error-key");
			SessionMessages.add(actionRequest, PortalUtil.getPortletId(actionRequest) + SessionMessages.KEY_SUFFIX_HIDE_DEFAULT_ERROR_MESSAGE);
			actionResponse.setRenderParameter("jspPage","/html/addknowledgebites.jsp");
		}
		else
		{
		...
		if (arrLength >= 1) {
			...
			logger.info("Knowledge Bite Created with ID : " + sop.getId());
	...
	ThemeDisplay themeDisplay =(ThemeDisplay)request.getAttribute(WebKeys.THEME_DISPLAY);
		...
		if(desc.equalsIgnoreCase("") || desc.length()<1)
		{
			System.out.println("ck editor data null>>>>>>");
