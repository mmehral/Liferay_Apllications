...
<%
		int count = EmployeeRefrenceLocalServiceUtil.getEmployeeRefrencesCount();
		System.out.println("Count : " + count);
		List<EmployeeRefrence> list = (List<EmployeeRefrence>) EmployeeRefrenceLocalServiceUtil
				.getEmployeeRefrences(0, count);
	...
	%>
<liferay-ui:header title="Show Refferal Details"
	backLabel="&laquo; Back to search" backURL="<%=backURL.toString()%>" />
	<liferay-ui:search-container delta="3"
	   total="<%=EmployeeRefrenceLocalServiceUtil.getEmployeeRefrencesCount()%>"
	...
	<liferay-ui:search-container-row modelVar="list1" className="EmployeeRefrence">
