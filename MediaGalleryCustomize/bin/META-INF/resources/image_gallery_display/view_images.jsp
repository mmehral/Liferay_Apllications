...
dlFileEntries = DLFileEntryLocalServiceUtil.getFileEntries(curFolder.getGroupId(), curFolder.getFolderId(),0, -1, -1, orderByComparator);
DLFileEntry dl=null;
for(int i=0;i<1;i++){
	dl=dlFileEntries.get(i);
}

System.out.println(dl.getFileEntryId());
System.out.println(dl.getStatus());

String url=themeDisplay.getPortalURL() + themeDisplay.getPathContext() + "/documents/" + themeDisplay.getScopeGroupId() + StringPool.SLASH + curFolder.getFolderId() +  StringPool.SLASH + HttpUtil.encodeURL(HtmlUtil.unescape(dl.getFileName()));
/* String url=themeDisplay.getPortalURL() + themeDisplay.getPathContext() + "/documents/" + themeDisplay.getScopeGroupId() + StringPool.SLASH + dl. getFileName(); */
...
%>
			...
			<div class="col-md-12 hel_box_panel">
					<div class="col-md-2 con_paddi">
<span class="con_img_cen"><img class="hel_img" alt="Chola" src="<%=url%>">
</span>
</div>

<div class="col-md-10"><span class="comp_text_head"><span class="abt_text_headcol"><%= curFolder.getName() %></span></span>

<br><%= curFolder.getDescription()%><br>


				
<a class="all_text_link" href="<%=viewFolderURL%>"><button class="con_btn_styl"><span class="con_more">More +</span></button></a>
</div>
